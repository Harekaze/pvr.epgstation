...
bool Rule::refresh()
{
    ...
    for (picojson::value& a : response.get<picojson::array>()) {
        ...
        if (p["reserve_titles"].is<picojson::array>() && !p["reserve_titles"].get<picojson::array>().empty() && p["reserve_descriptions"].is<picojson::array>() && !p["reserve_descriptions"].get<picojson::array>().empty()) {
            XBMC->Log(ADDON::LOG_DEBUG, "Skipped - multi reserve pattern rule: %d", i);
            continue;
        }
        if (p["ignore_titles"].is<picojson::array>() && !p["ignore_titles"].get<picojson::array>().empty() && p["ignore_descriptions"].is<picojson::array>() && !p["ignore_descriptions"].get<picojson::array>().empty()) {
            XBMC->Log(ADDON::LOG_DEBUG, "Skipped - multi ignore pattern rule: %d", i);
            continue;
        }
        if (p["reserve_titles"].is<picojson::array>() && !p["reserve_titles"].get<picojson::array>().empty() && p["ignore_descriptions"].is<picojson::array>() && !p["ignore_descriptions"].get<picojson::array>().empty()) {
